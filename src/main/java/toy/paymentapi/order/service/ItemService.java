...
public class ItemService {
     ...
     * - Integer itemSize : 한 페이지에 담을 상품수
     *
     * return
     * - ItemPageableDto :
     *      - 전체 상품 수
    ...
    public ItemPageableDto pagedItemSortByRegisteredAt(int pageNum, int itemSize) {
        ...
        Page<Item> itemPage = getItemInPage(pageRequest);
         ...
         */
        if(totalItems == 0){
            throw ErrorCode.throwNotFoundItem();
        ...
        if (!itemPage.hasNext() && !itemPage.hasContent()) {
            Page<Item> previousPage = getItemInPage(pageRequest.previousOrFirst());
            itemList = convertToList(previousPage);
            currentPageNum = previousPage.getNumber();

        } else {
            itemList = convertToList(itemPage);
    ...
    * 페이징 정보로 상품 조회
    **/
    private Page<Item> getItemInPage(Pageable pageRequest) {
        return itemRepository.findAll(pageRequest);
    }

    /**
    * Page<Item> -> List<ItemDto>
    **/
    private List<ItemDto> convertToList(Page<Item> itemPage) {
